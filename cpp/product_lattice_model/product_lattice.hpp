...
class Product_lattice{
	...
	Product_lattice(int n_atom, int n_variant, double* pi0) : atom_(n_atom), variant_(n_variant){
        ...
        post_probs_ = new double[total_state()];
		prior_probs(pi0);
		test_ct_ = 0;
	}
	...
	Product_lattice(const Product_lattice &other, int assert){
		...
		test_count(other.test_ct_);
        // post_probs = new double[total_state()];
	...
	virtual ~Product_lattice(){
		delete[] post_probs_;
		delete[] clas_stat_;
		std::cout << "Delete Lattice Model" << std::endl;
	}
	...
	void update_probs(int experiment, int response, double thres_up, double thres_lo);
	...
	virtual double* calc_probs(int experiment, int response) = 0;
