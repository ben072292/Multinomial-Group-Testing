...
Product_lattice::Product_lattice(int n_atom, int n_variant, double* pi0) : curr_subj_(n_atom), variant_(n_variant){
	pos_clas_ = 0;
	neg_clas_ = 0;
...
int* Product_lattice::get_up_set(int state, int* ret) const{
    int *add_index = new int[curr_atoms() - __builtin_popcount(state)];
    int counter = 0, i, index;
    for(i = 0; i < curr_atoms(); i++){
        index = (1 << i);
        if((state & index) == 0){
...
// 	int n = curr_atoms()-1; // shared variables
...
// 		#pragma omp critical // curr_atoms is small so omit contention overhead
//         {
...
double Product_lattice::get_prob_mass(int state) const{
    double ret = 0.0;
	int n = curr_atoms() - __builtin_popcount(state), pow_set_size = 1 << n, j, temp;
...
bool Product_lattice::is_classified() const{
	return __builtin_popcount(pos_clas_ + neg_clas_) == curr_atoms();
}

double* Product_lattice::calc_probs(int experiment, int response, double** dilution){
	int total_stat = 1 << (curr_subj_ * variant_);
