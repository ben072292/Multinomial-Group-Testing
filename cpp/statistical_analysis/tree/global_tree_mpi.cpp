 ...
 */
Global_tree_mpi::Global_tree_mpi(Product_lattice *lattice, bin_enc ex, bin_enc res, int k, int curr_stage) : Global_tree_mpi(lattice, ex, res, curr_stage)
{
    if (!lattice->is_classified() && curr_stage < _search_depth)
    {
        ...
        for (int re = 0; re < (1 << variants()); re++)
        {
            ...
            delete p;
            _children[re] = new Global_tree_mpi(p1, ex, re, k, _curr_stage + 1);
        }
    }
    else
 ...
 */
Global_tree_mpi::Global_tree_mpi(Product_lattice *lattice, bin_enc ex, bin_enc res, int k, int curr_stage, bool perf) : Global_tree_mpi(lattice, ex, res, curr_stage)
{
    ...
    if (!lattice->is_classified() && curr_stage < _search_depth)
    {
        ...
        for (int re = 0; re < (1 << variants()); re++)
        {
            ...
            _children[re] = new Global_tree_mpi(p1, ex, re, k, _curr_stage + 1, true);
        }
    }
    else
    { // clean in advance to save memory
...
Global_tree_mpi::Global_tree_mpi(const Tree &other, bool deep) : Global_tree(other, false)
{
    if (deep)
    {
        if (other.children() != nullptr)
        {
            ...
            {
                _children[i] = new Global_tree_mpi(*(other.children()[i]), deep); // TBD: this downcast is problematic, use virtual clone and create functions
            }
        }
    }
}

void Global_tree_mpi::MPI_Global_tree_Initialize(int subjs, int k)
{
