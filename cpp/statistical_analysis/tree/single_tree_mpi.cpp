...
Single_tree_mpi::Single_tree_mpi(Product_lattice* lattice, int ex, int res, int k, int cur_stage, double thres_up, double thres_lo, int stage, double** dilution, int rank, int world_size, MPI_Op* halving_op) : Single_tree_mpi(lattice, ex, res, cur_stage){
    if (!lattice->is_classified() && cur_stage < stage) {
        ...
        for(int re = 0; re < (1 << lattice->variant()); re++){
            if(re != (1 << lattice->variant())-1){
                ...
                children_[re] = new Single_tree_mpi(p, halving, re, k, cur_stage_+1, thres_up, thres_lo, stage, dilution, rank, world_size, halving_op);
            }
