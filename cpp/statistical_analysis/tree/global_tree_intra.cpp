...
Global_tree_intra::Global_tree_intra(Product_lattice *lattice, bin_enc ex, bin_enc res, int k, int curr_stage, std::chrono::nanoseconds halving_times[]) : Global_tree_intra(lattice, ex, res, curr_stage)
{
    ...
    if (!lattice->is_classified() && curr_stage < _search_depth)
    {
        ...
        for (bin_enc re = 0; re < (1 << variants()); re++)
        {
            ...
            p->update_metadata_with_shrinking(_thres_up, _thres_lo);
            _children[re] = new Global_tree_intra(p, ex, re, k, _curr_stage + 1, halving_times);
        }
    }
    else
    { // clean in advance to save memory
