...
int main(int argc, char *argv[])
{
    ...
    // Initialize product lattice MPI env
    Doubly_linked_global_tree_mpi::MPI_Global_tree_Initialize(subjs, search_depth, 1);
    ...
    // Fusion tree
    // Global_tree *tree = new Doubly_linked_global_tree_mpi(p, -1, -1, 1, 0, thres_up, thres_lo, search_depth, pi0, dilution, 0.01, 0.0, 1e-6, nullptr);
    // Global tree without perf
    // Global_tree* tree = new Doubly_linked_global_tree_mpi(p, -1, -1, 1, 0, thres_up, thres_lo, search_depth, dilution, nullptr);
    // Global tree with perf
    Doubly_linked_global_tree_mpi *tree = new Doubly_linked_global_tree_mpi(p, -1, -1, 1, 0, thres_up, thres_lo, search_depth, dilution, true, nullptr);
    ...
    if (rank == 0)
    {
        std::cout << "10 Unclassified Test Sequences:" << std::endl;
        std::cout << "Stage, Pooled Subjects (1 means pooled), Outcome (1 means negative)" << std::endl;
        std::vector<const Global_tree *> *leaves = new std::vector<const Global_tree *>;
        std::vector<const Global_tree *> sequence;
        tree->find_unclas_stat(tree, leaves, -1.0);
        const Global_tree *cursor;
        for (size_t i = 1; i <= 10; i++)
        {
            int index = (*leaves).size() / 20 * i;
            if ((*leaves)[index]->curr_stage() == search_depth)
            {
                cursor = (*leaves)[index];
                while (cursor->ex_res() != -1)
                {
                ...
                {
                    std::cout << j + 1 << ", " << to_binary(sequence[sequence.size() - j - 1]->ex(), subjs) << "," << to_binary(sequence[sequence.size() - j - 1]->ex_res(), variant) << std::endl;
                }
                sequence.clear();
                std::cout << std::endl
                          << std::endl;
    ...
    Doubly_linked_global_tree_mpi::MPI_Global_tree_Free();
